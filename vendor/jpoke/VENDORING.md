...
## 自動同期(GitHub Actions, 2026-09-11導入)
...
上記ワークフローが動かない場合や、手元の未リリース版を試したい場合は以下を手動で行う。
...
3. `npm run build:master-data` を実行し、エラーなく完走することを確認する。
   - wheelファイル名には内容のsha256ハッシュが自動で埋め込まれる(2026-07-30導入)ため、
     jpokeのバージョン番号(`pyproject.toml`)を上げなくても、中身が変わればURLが変わり、
     ブラウザのService Workerキャッシュ(cache-first)が古いwheelを握ったままになる事故を
     気にする必要はない。手動でキャッシュ名やバージョンを更新する作業は不要。
