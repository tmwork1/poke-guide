...
class Battle:
    ...
    def calc_move_base_power(self,
        ...
        基礎威力の解決・かたやぶり適用）を施し、けたぐり・はきだす・アクロバット・
        アシストパワー・ウェザーボール等、技固有の状況で基礎威力自体が変わる
        技の変動までを含む基礎威力を返す。特性・持ち物・天候等の
    ...
    def calc_move_power(self,
        ...
        基礎威力変動、特性・持ち物・天候等の威力補正（ON_CALC_POWER_MODIFIER）、
        テラスタル時の威力60底上げまで含めた、ダメージ式に入る威力を返す。
        技固有の基礎威力変動のみ（特性・持ち物補正を含まない値）は
        :meth:`calc_move_base_power` を使用する。
