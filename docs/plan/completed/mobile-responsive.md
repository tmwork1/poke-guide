...
1. **CSS論理幅412pxを基準に、カードサイズの最大値を定義する。**
...
### 3.1 基準寸法(412px)— カードの最大サイズはグリッドごとに別

主要端末のCSS論理幅は **402〜412px**(iPhone 16 Pro = 402、Pixel 9/10 Pro = 412)。**設計基準幅を 412px** とし、402 / 390(小型端末)でも破綻しないことを条件にする。
...
:root {
  ...
  --box-card-size-max: 130px;   /* 412pxで3列に並べたときの実測値 */
