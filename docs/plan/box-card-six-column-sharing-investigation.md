...
# フェーズ3: 6列タイルからカード背景を外す

方針確定: 2026-09-11

## 背景

6列表示に出ているのは立ち絵と持ち物アイコンだけで、カードの背景・枠・影を使ってまで
1体ずつを分離する必要がある情報を持っていない。背景を外して絵とアイテムだけにする。

## やること

### 1. 6列タイルのカード背景を非表示にする

対象:

- `/box` 一覧ページの圧縮表示(6列)
- `/team/[id]` の編成タブ・相性タブ・類似チームタブの6列表示
- `.team-mate-card` を使う残りすべて(`/data` の上位チーム圧縮表示、`/team` 一覧の
  圧縮チームカード、持ち物再分配モーダルのメンバー6枠、個体編集パネルの所属チーム6枠)。
  共有コンポーネントなので画面ごとに見た目を分けない。

`.team-mate-card` と `/box` 圧縮表示の `.box-card` から、通常状態の
`background-color` / `background-image` / `border` / `box-shadow` を外し、立ち絵と
持ち物アイコンだけが並ぶ状態にする。

外さないもの:

- 選択・ホバー・フォーカス時の枠(`.is-selected` / `:hover` / `:focus-visible` の
  `border-color` + `outline`)。操作可能であることが分からなくなるため残す。
- 空き枠(`.team-mate-card--empty`)の枠線。枠が無いとスロット番号が宙に浮く。
- `/box` 圧縮表示の削除ボタン、長押し削除モードの見た目。
- 再分配モーダルの `.is-locked` 表示、個体編集パネルの `.is-current` マーカー。

タイルの占有サイズ(`aspect-ratio: 1/1`、`--compact-thumb-card-size-max`、
グリッドの `gap`)は変えない。背景が消えるだけで、絵の大きさと配置は現状どおりにする。

### 2. 上位チームカードを「見出しカード + 横長カード」の2枚構成にする

**圧縮表示(6列タイル)のときだけ**適用する。展開表示(名前・技を持つボックスカード)は
現行のまま1枚のカードに収める。

- 順位・トレーナー名・シーズン・レート・構築記事リンクを持つ見出しカードはそのまま1枚のカード。
- その下に、6体全員を収める横長のカードを**別のカードとして**追加する。枠線・背景・角丸は
  通常のカードと同じトークンを使い、見出しカードとの間には通常の `gap` を入れる。
- 背景を失った6枚のタイルは、この横長カードの中に並ぶ。

対象は圧縮表示を持つチームカード全部。`renderTopBuildCard()` を使う3画面
(`/box/ranked`、`/data`、`/team/[id]` の類似チームタブ)に加え、`/team` 一覧の
圧縮チームカードにも同じ横長カードを入れる。どの画面でも「背景なしのタイルが
容れ物の中に並ぶ」形にそろえる。

## 受け入れ条件

- `/box` 圧縮表示・`/team/[id]` の3タブで、タイルに背景・枠・影が出ていない
  (`getComputedStyle` の `background-color` が透明、`border-width` が0、`box-shadow` が none)。
- 選択状態・ホバー・空き枠の枠線は従来どおり見える。
- 上位チームカードの圧縮表示が、見出しカードと6体の横長カードの2枚に分かれて見える。
- 展開表示の上位チームカードは従来どおり1枚のまま。
- タイルの寸法・列数・`gap` が変更前と一致する。
- `/box`、`/data`、`/team`、`/team/[id]`、`/box/ranked` を390pxと広幅で確認し、
  横スクロールを発生させない。
