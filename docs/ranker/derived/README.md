...
| 構築記事があるチーム | 300 |
| 記事から個体情報が取れたチーム | 239 |
| 記事があるのに何も取れなかったチーム | 61(x.com 29 / YouTube 3 / 本文が画像のみ 等) |
| 個体情報が入ったメンバー | 1352(pokesol 513 / LLM 839) |
| 技が入ったメンバー | 1300 |
| 努力値が入ったメンバー | 853 |
| 性格 / 特性が入ったメンバー | 1051 / 1056 |

「記事があるのに取れなかった61件」は**抽出の失敗ではない**。構築記事の多くは個体情報を
画像(スクリーンショット)で貼っており、本文テキストには数値が存在しない。
Haikuが「本文に無い」と判定した20件をSonnetで読み直しても、実際に本文に記載があったのは
1件だけだった。テキストからの抽出としてはほぼここが上限で、増やすには画像OCRが要る。

努力値が853件しか無いのは、記事が数値ではなく「HB:特化リザードンのニトロチャージ耐え」
...
- **`llm`** — それ以外(はてなブログ/note/blog.naver.com 等)は散文なので、Haiku subagentに `scripts/ranker/EXTRACTION_SPEC.md` の仕様で読ませている。LLMの出力はそのまま信じず、`build_ranked_teams.py` で次の4段階の検算にかけてから採用する。
  ...
  4. **スロットの割り当て直し** — LLMは個体を**記事に出てくる順**で番号付けしがちで、公式ランキングの並び順とずれる(実際にM-1 2位でアーマーガアとハラバリーの技が入れ替わっていた)。`vendor/jpoke` のlearnsetsと特性表を使い、「その種族が覚えられる技か」で総当たり的に割り当て直す。**812体中102体がこれで直っている。** 直してもなお半分以上の技が非整合な個体は値を残したまま `confidence` を `low` に落とす。

  落ちた値は `extraction-report.json` の `rejected_by_validation` に残る。
  検算後の技の整合率は LLM由来 2204/2241 (98.3%)、pokesol由来 1983/2017 (98.3%) で、
