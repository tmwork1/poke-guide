...
一方、ウォームアップ後にはじめて見えるようになった実在の課題もある:

| 画面/操作 | 実測 | 対応する発見 |
|---|---|---|
| ダメージ表を初回表示 | 6243ms / 1.25x 🟡 | box.md 発見A(横断課題6のyield漏れ) |
| 編成タブの枠を選択 | 401ms / 1.34x 🟡 | team.md 発見A(250msの無条件クリック遅延) |
| もちもの入替の枠を選択 | 364ms / 1.21x 🟡 | 同上 |

**現時点で最も確度が高い改善対象はこの3行**で、いずれも原因がコードレベルで特定済み(フェーズ1の項目)。

**未解決**: `/box/ranked`(上位チームを表示)は3試行中2試行が120秒タイムアウトで落ちた
(終点 `#ranked-data-panel .box-ranked-results, #ranked-data-status:not([data-state='loading'])` に
到達しない)。表の414msは通った1試行だけの値なので信用できない。box-insight.md 発見B・横断課題2の
「自動継続読み込み」が関係している可能性があり、次に調べるべき対象。
