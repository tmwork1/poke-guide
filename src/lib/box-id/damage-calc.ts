...
export const DAMAGE_WEATHERS = [
	...
	{ value: "すなあらし", label: "すなあらし", icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9.6 4.6a2 2 0 1 1 1.4 3.4H3"/><path d="M12.6 19.4a2 2 0 1 0 1.4-3.4H3"/><path d="M17.7 7.7a2.5 2.5 0 1 1 1.8 4.3H3"/></svg>` },
...
if (opponentNotesSection) {
	...
	// エンジンが返す cumulativeDamage(回復・スリップを含まない打点合計の厳密な
	// 最小/最大)を使い、無い場合は技ごとの最小/最大を単純加算する。
	// 本体は damage-summary.ts の computeCumulativeDamage(圧縮表示と共有。上のimport参照)。
	// ここは行から有効な技列の件数を数えるだけの薄い層。
	...
	function renderRow(row: DamageRowState): HTMLElement {
		...
		// 2段目(buildFields)の末尾に置く。特性・テラス・アイテムと同じ「その他の情報」の
		// まとまりとして中央寄せ表示する。
		buildFields.appendChild(readonlyEvGrid);
