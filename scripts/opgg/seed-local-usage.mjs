 ...
 *
 * 【実データが見たいときはこれを使わない】
 *   画面の中身を実際のチャンピオンズ環境として確認したい場合は
 *   `node scripts/opgg/fetch-champions-usage.mjs --local` を使う(op.gg から全種族を取得して
 *   ローカルKVへ投入する。数分かかる)。このスクリプトが作るのは合成データなので、
 *   採用率・構成の中身は本物ではない。用途はパフォーマンス計測の再現性確保に限る。
 *
 * 【なぜ実データを取りに行かないのか】
