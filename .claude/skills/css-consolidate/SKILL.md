...
**原則Coordinator自身は Edit/Write でコードを書きません**(このリポジトリの一貫した方針。Coordinatorの仕事は分割方針の確定・diff確認・検証・commit)。**委託先はcodexが使えるなら `codex` skill 経由の `codex exec` を優先し**、疎通不良のときだけ sonnet の Agent tool にフォールバックします。

**ただし移動対象が小規模(1ファイル内の数ルールのみ)な場合は、Coordinatorが直接移してもよい。** 一字一句変えない単純な移動で、委任のプロンプト作成・結果検証の往復の方が作業そのものより重くなる規模ならCoordinator自身が行う。この場合も下記「4. 検証する」のスクリーンショット比較は省略しない。
