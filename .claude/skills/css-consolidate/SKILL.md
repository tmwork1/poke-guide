...
**Coordinator自身は Edit/Write でコードを書きません**(このリポジトリの一貫した方針。Coordinatorの仕事は分割方針の確定・diff確認・検証・commit)。**委託先はcodexが使えるなら `codex` skill 経由の `codex exec` を優先し**、疎通不良のときだけ sonnet の Agent tool にフォールバックします。
