...
- **そのルールを説明しているコメントも、ルール本体と一緒に新ファイルへ移すこと。** 埋め込み `<style>` には「UI改修依頼(〜)」「なぜこの値か」を説明するコメントがルール直前・ブロック内に大量に付いている(このプロジェクトの一貫した書き方)。ルールだけ抜き取ってコメントを `.astro` 側に置き去りにすると、(1) 新CSSファイルが「値の羅列だけで意味不明」になり、(2) `.astro` 側に「直後の対象が既に存在しないコメント」が宙に浮いて残る、という二重の劣化が起きる(2026-08-11に実際に発生し、Coordinatorが検知して委譲し直した事故)。**完了報告の前に、移動元の `.astro` に「直前のルールが既に無いのにコメントだけ残っている」箇所が無いか自分で確認すること。** 逆に、複数のルールにまたがる文脈コメント(例: グリッド全体の設計意図の説明)は、対象ルール群の直前にそのまま置けばよい(分割しない)
- 新ファイルの先頭に、対象を1行で説明する短い日本語コメントを入れること。既存ファイルの書き方に揃える(例: `/* /box: モバイル用のポケモン一覧画面。カード本体の見た目は box-card.css に分離する。 */`)
- **対象クラスがJSで動的生成される要素の場合、ルールごとのコメントに「具体的にどの要素か」を書くこと。** `.astro` の静的markupと違い、CSSファイル単体を見てもセレクタがどの要素を指すか分からない(クラス名が抽象的だと特定できない)。生成元の関数・ファイルを踏まえ、「ニックネーム表示」「わざ一覧のタイプアイコン」のように要素の役割を一言添える(実例: `src/styles/team-pokemon-card.css` の `.pokemon-preview-*` 一式に2026-08-11追記。生成元は `src/pages/team/[id].astro` の `document.createElement` によるカード組み立て処理)
...
- **「セレクタ・プロパティ・値を一字一句変えていないか」はスクリーンショットより先に機械照合できる。** 前後のCSSからコメントを除去したうえで `セレクタ → 宣言の集合(ソートして正規化)` のリストに変換し、差分を取れば空白・改行・並び順を無視して意味的な変更点だけが浮かぶ(実例: 2026-08-11、`team-pokemon-card.css` の再整形でこの方法により34ルールの完全一致、および孤児ルール削除後に生存ルールの計算結果が変わっていないことを確認した)
- 手順:
...
- `src/styles/team-pokemon-card.css` — JS動的生成markup(`src/pages/team/[id].astro`)向けに要素特定コメントを追記した例。過去のカード刷新で消し忘れられた孤児ルールを検出・削除した実例でもある(2026-08-11)
