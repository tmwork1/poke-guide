...
- 分割案が決まらない/対象が大きい場合は、埋め込みstyleの棚卸し(セレクタの一覧と役割の分類)をサブエージェントに委託してから決めてよいです。**codexが使えるなら`codex exec --sandbox read-only`を優先し**、疎通不良のときだけExploreエージェント(sonnet)にフォールバックします
- **対象のクラスが `.astro` の静的markupではなくJSで動的生成される(`document.createElement`等)場合、既存のCSSに「そのクラスが今も実際の要素に使われているか」を疑う。** マークアップ刷新(JS側のクラス変更)とCSS側の追随は別コミットになりがちで、旧デザインのルールが消し忘れられたまま残ることがある(実例: `src/styles/team-pokemon-card.css` に旧カードデザインの `.card-artwork`/`.card-moves-grid`/`.card-move-chip`/`.team-overview-fallback` 等が、`src/pages/team/[id].astro` 側の刷新(`ca84e5d`)後も孤児ルールとして残っていた)。切り出し・整理の前に対象クラス名を生成元ファイルでgrepし、ヒットしなければそのルールは切り出さず削除を提案する
...
**原則Coordinator自身は Edit/Write でコードを書かず、サブエージェントに委託します**(Coordinatorの仕事は分割方針の確定・diff確認・検証・commit)。委託先はcodexが使えるなら `codex` skill 経由の `codex exec` を優先し、疎通不良のときだけ sonnet の Agent tool にフォールバックします。**ただし移動対象が小規模(1ファイル内の数ルールのみ)で、委任のプロンプト作成・結果検証の往復の方が作業そのものより重くなる場合は、Coordinatorが直接移してよい**(この場合も下記「4. 検証する」のスクリーンショット比較は省略しない)。
...
- **そのルールを説明しているコメントも、ルール本体と一緒に新ファイルへ移すこと。** 埋め込み `<style>` には「UI改修依頼(〜)」「なぜこの値か」を説明するコメントがルール直前・ブロック内に大量に付いている(このプロジェクトの一貫した書き方)。ルールだけ抜き取ってコメントを `.astro` 側に置き去りにすると、(1) 新CSSファイルが「値の羅列だけで意味不明」になり、(2) `.astro` 側に「直後の対象が既に存在しないコメント」が宙に浮いて残る、という二重の劣化が起きる(実際に発生し、Coordinatorが検知して委譲し直した事故がある)。**完了報告の前に、移動元の `.astro` に「直前のルールが既に無いのにコメントだけ残っている」箇所が無いか自分で確認すること。** 逆に、複数のルールにまたがる文脈コメント(例: グリッド全体の設計意図の説明)は、対象ルール群の直前にそのまま置けばよい(分割しない)
- 新ファイルの先頭に、対象を1行で説明する短い日本語コメントを入れること。既存ファイルの書き方に揃える(例: `/* /box: モバイル用のポケモン一覧画面。カード本体の見た目は box-card.css に分離する。 */`)
- **対象クラスがJSで動的生成される要素の場合、ルールごとのコメントに「具体的にどの要素か」を書くこと。** `.astro` の静的markupと違い、CSSファイル単体を見てもセレクタがどの要素を指すか分からない(クラス名が抽象的だと特定できない)。生成元の関数・ファイルを踏まえ、「ニックネーム表示」「わざ一覧のタイプアイコン」のように要素の役割を一言添える(実例: `src/styles/team-pokemon-card.css` の `.pokemon-preview-*` 一式に追記済み。生成元は `src/pages/team/[id].astro` の `document.createElement` によるカード組み立て処理)
...
- **スクリーンショット比較では「使われていないルール」は検出できません**(見た目に影響しないぶん差分にも出ないため)。移動・整理対象のクラス名を生成元(`.astro`のテンプレート、またはJSの`document.createElement`箇所)でgrepし、ヒットしないセレクタは孤児ルールとして削除を検討する。見た目非変更の原則から一見外れるが、対応する要素が存在しない以上「見た目を変えない」の対象外であり、削除が妥当な整理(実施前にユーザーに確認する)
- **「セレクタ・プロパティ・値を一字一句変えていないか」はスクリーンショットより先に機械照合できる。** 前後のCSSからコメントを除去したうえで `セレクタ → 宣言の集合(ソートして正規化)` のリストに変換し、差分を取れば空白・改行・並び順を無視して意味的な変更点だけが浮かぶ(実例: `team-pokemon-card.css` の再整形でこの方法により34ルールの完全一致、および孤児ルール削除後に生存ルールの計算結果が変わっていないことを確認した)
- 手順:
...
- `src/styles/team-pokemon-card.css` — JS動的生成markup(`src/pages/team/[id].astro`)向けに要素特定コメントを追記した例。過去のカード刷新で消し忘れられた孤児ルールを検出・削除した実例でもある
