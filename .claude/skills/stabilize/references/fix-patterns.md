...
## ⚠️ 最初に確認すること: それは dev server の偽物ではないか

**`npm run dev` で測った揺れの大半は、アプリのコードではなく Vite の dev client が作っている。**
Astro dev は各CSSを `<style>` としてHTMLに直接埋め込むが、**JSが立ち上がると Vite の dev client が
その `<style>` の中身を全部入れ替える**(`<head>` を `--watch` すると `style childList "-1" → "+1"` が
一斉に並ぶ)。その入れ替えの直後にページ全体が一度リフローする。本番ビルドではCSSは `<link>` で
配られるのでこれは起きない。

**見分け方(2つとも当てはまったら dev の偽物):**

```bash
npm run probe -- --page <path> --size 390x844 --theme dark --no-js --cls   # → CLS 0 になるか
npm run probe -- --page <path> --size 390x844 --theme dark --cls --watch "head"
```

1. `--no-js` で **CLS 0**(Vite dev client が動かないので偽物は消える)
2. `--watch "head"` の `style childList "-1" → "+1"` の時刻の**直後**にシフトが立っている

この2つが揃ったら**アプリ側に直すものは無い**。dashboardの「対応できない・許容している揺れ」に
記録して次へ進む。実測(2026-09-10)では `/box/<id>` の残り0.07も `/team/<id>` の0.02も、
すべてこれだった。**この確認を飛ばすと、存在しないバグを追ってサブエージェント1回分を丸ごと無駄にする。**

本物かどうかを最終確認したいときは `npm run preview`(本番ビルド)で測る。
