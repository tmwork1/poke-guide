...
2. **`LethalHitResult.__add__`/`subtract_dist`の内部挙動への依存**(`calc_lethal_sequence_json`全体): 打点とHPを分けて集計するロジック・単調性クランプは、jpokeの`jpoke/core/lethal.py`の非公開実装詳細(`_lethal_loop`の打ち切り仕様、`__add__`のダメージ/HP合成規則)への深い依存。ここが変わっても型は壊れず、累計致死率・打点の数値だけがずれる。**(2026-07-28時点)** §6の一致確認(`stats-lethal-sequence.spec.ts`)がこの経路をカバーするようになったため、この種の変化はテストで検出できるようになった。**(2026-07-30追記)** 技列全体の累計(lethal/cumulativeDamage)は`__add__`ベースの独立合成をやめ、jpokeの`calc_lethal(resume_from=...)`(新規追加、`.claude/skills/jpoke/references/damage-calc.md`§10参照)に置き換えた。`__add__`は「各技カード単体の参考値」(perAttackLethalの自己合成)にのみ残っている。
3. **~~`tests/e2e/fixtures/generate_expected.py`のBOOTSTRAP_PYTHON複製が古い~~ → 2026-07-28 UI改善ラウンド22 22-E-1/22-E-2で解消済み**(§6参照)。複製をやめてBOOTSTRAP_PYTHONを直接execする方式に変更し、`calcStats`/`calcLethalSequence`の一致確認も追加した。`vendoring更新手順(VENDORING.md`手順4)の前提「jpoke更新後にnpm testで検出できる」は、`npm run test:e2e`(`test:e2e:update-fixtures`で期待値再生成後)まで含めれば、`/box/[id].astro`が実際に使う計算経路(`calcStats`/`calcLethalSequence`)もカバーするようになった。**(2026-07-30実例)** この経路が実際に効いた: `resume_from`取り込み直後、`jpoke.utils.lethal_dist.State`から`attacker_boosts`等4フィールドが削除されたことに`pyodide-engine.ts`の`_clamp_hp_dist_min0`が追随しておらず、`generate_expected.py`実行時に`AttributeError`で即座に検出できた(型チェック(`npm run build`のastro check)では検出されない実行時エラー。詳細はdamage-calc.md§10)。
...
### 初期化後の1回あたりの計算コスト(2026-08-02 実測、耐久調整機能の設計時に追記)

**初期化さえ終わっていれば `calcLethalSequence()` 1回は数十ms**で、まとめて数百回呼ぶ設計が現実的に成立する。`CALC_DEBOUNCE_MS = 600`(`shared-core.ts`)という既存のデバウンス値から「1回が重い」と誤解しやすいが、**実際に重いのは初期化だけ**である。

実測条件: Windows側 `astro dev`(localhost:4321)/ Playwright(Chromium)/ 個体 `c8680844-...`(メガリザードンX)を防御側、フシギバナ(キョダイ)の「でんきショック」を攻撃側。エンジンはウォーム済み(ページのカード計算が完了した後)。

| 技列の長さ(attacks件数) | 所要時間 |
|---|---|
| 1 | 9ms |
| 2 | 16ms |
| 3 | 22ms |
...
同じ形の呼び出しを連続5回(防御側の努力値だけ変える)では 20〜22ms で安定。**攻撃回数にほぼ比例して伸びる**(致死率計算が畳み込みを重ねるため)ので、確定数が出ない条件で10発まで延長するような使い方はこの表より重くなる点に注意。

- **メモ化のキーは努力値ではなく実数値にする**: 努力値が違っても実数値が同じなら結果は同じ。0〜32スケールの努力値は1段ごとに実数値が1上がるとは限らず、重複が出る。
- **性格は耐久計算では (B補正, D補正) の組でしか効かない**ため、25性格ではなく7通り((1,1)(1.1,1)(1,1.1)(0.9,1)(1,0.9)(1.1,0.9)(0.9,1.1))を評価して展開すればよい。ただし**「こだいかっせい」「クォークチャージ」「ブーストエナジー」は最も高い実数値がどれかで効果が変わる**ため、これらを持つ個体ではA/C/Sの補正差も結果に影響する(7クラスへのまとめが成立しない)。

### dev serverではページ内から `import('/src/lib/...')` でモジュールを直接叩ける(2026-08-02 実測)

`page.evaluate()` の中で `await import('/src/lib/pyodide-engine.ts')` のように**ソースのパスをそのまま**動的importすると、Viteが同一URLで解決するため**ページが既に初期化したのと同じシングルトン**が取れる(実測: `isEngineReady()` が `true` を返した)。`shared-core.ts` のブリッジ(`getBulkAdjustBridge()` 等)も同じ方法で取れる。

エンジンやブリッジの検証スクリプトを書くとき、**UIを操作せずに内部APIだけを叩ける**ので、`/box/[id]` の自動保存を発火させずに済む(データ事故を構造的に避けられる)。ただしこれは**dev serverでのみ**成立する(ビルド後はバンドルされてこのパスは存在しない)。
